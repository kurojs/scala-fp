...
class NonEmpty(elem: Tweet, left: TweetSet, right: TweetSet) extends TweetSet:
  ...
  override def mostRetweeted: Tweet =
    ...
    if !left.empty && !right.empty then {
      if leftMostRetweeted.retweets > elem.retweets && leftMostRetweeted.retweets > rightMostRetweeted.retweets then
        leftMostRetweeted
      else if rightMostRetweeted.retweets > elem.retweets && rightMostRetweeted.retweets > leftMostRetweeted.retweets then
        rightMostRetweeted
      else
        elem
...
object GoogleVsApple:
  ...
  lazy val googleTweets: TweetSet =
    tweets.filter(tweet => {
      tweet.text.contains(google)
    })

  lazy val appleTweets: TweetSet =
    tweets.filter(tweet => {
      tweet.text.contains(apple)
    })
